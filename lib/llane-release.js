...
async function release() {
  ...
  try{
    ...
    if(llaneConfig.token[md5(`${GIT_REPOSITORY_URL}${WWWROOT}${projectConfig.projectPath}`)] !== 1){
    ...
    var { stdout, stderr } = await exec(`scp -r  ${buildPath} root@192.168.5.17:${projectPath}`);
    if(stdout) log('stdout:', stdout);
    if(stderr) throw stderr;

    if(argv[0] === "prod"){
      var { err, stdout, stderr } = await exec(`ssh root@192.168.5.17 "ssh -p 16000 JYSD@218.206.205.10 \"mkdir -p ${projectPath}\""`);
      if(stdout) log('stdout:', stdout);
      if(stderr) throw stderr;

      var { stdout, stderr } = await exec(`ssh root@192.168.5.17 "scp -P 16000 -r ${projectPath}/* JYSD@218.206.205.10:${projectPath}"`);
      if(stdout) log('stdout:', stdout);
      if(stderr) throw stderr;
