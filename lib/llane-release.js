...
async function release() {
  ...
  noticeParams.public_path = `http:${argv[0] === "prod" ? WWWROOT : QAROOT}/${projectConfig.projectPath.replace(sepReg, '')}`;
  ...
  try{
    ...
    if(argv[0] === "prod"){
      ...
      var { stdout, stderr } = await exec(`ssh root@192.168.12.17 "find ${projectPath} | grep -E \\"\\.(js|html)$\\" | xargs sed -i -e \\"s/192.168.12.17/static.joinuscn.com/g\\" "`);
      if(stdout) log('stdout:', stdout);
      if(stderr) throw stderr;
    ...
    var { stdout, stderr } = await exec(`ssh root@192.168.12.17 "find ${projectPath} | grep -E \\"\\.(js|html)$\\" | xargs sed -i -e \\"s/static.joinuscn.com/192.168.12.17/g\\" "`);
    if(stdout) log('stdout:', stdout);
    if(stderr) throw stderr;
